...
class Trainer(object):
    ...
    @staticmethod
    def plot_points(points, scores, index, name):
        ...
        for batch, cloud in enumerate(points):
            score = scores[batch]
            i = index[batch].item()
            with open(save_folder + "/{}.csv".format(str(i)), 'w+') as f:
                for point_index in range(cloud.shape[-1]):
                    f.write("{},{},{},{}\n".format(cloud[0, point_index], cloud[1, point_index],
                                                   cloud[2, point_index], score[point_index].item()))
